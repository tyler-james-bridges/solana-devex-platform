...
async function scanForSigningPatterns() {
  ...
  try {
    ...
    console.warn(chalk.yellow(`⚠️ Could not scan signing patterns: ${error.message}`));
  }
...
async function scanForFeePatterns() {
  ...
  try {
    ...
    console.warn(chalk.yellow(`⚠️ Could not scan fee patterns: ${error.message}`));
  }
...
function displayRiskAnalysis(risks) {
  ...
  if (allIssues.length === 0) {
    ...
    console.log(chalk.yellow(`\\n⚠️ Found ${allIssues.length} security issues:`));
...
function getStatusIcon(severity) {
  switch (severity) {
    ...
    case 'medium': return chalk.yellow('⚠️ Medium Risk');
